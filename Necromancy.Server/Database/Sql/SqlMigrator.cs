...
namespace Necromancy.Server.Database.Sql
{
    ...
    {
        private static readonly ILogger _Logger = LogProvider.Logger(typeof(SqlMigrator));
        ...
        public void Migrate(string migrationsFolder)
        {
            ...
            SortedList<long, string> versionFileDict = new SortedList<long, string>();
            /* Prepare migration files, ordered by version number. */
            foreach (string f in Directory.GetFiles(migrationsFolder, "*.sql"))
            {
            ...
            if ((versionNumber = _database.version) > initialVersion) _Logger.Debug($"Database db.sqlite version is updated to {versionNumber}.");
        }

        /// <summary>
        ///     Apply all migrations in-order per sql file, and update the DB version.
        /// </summary>
        /// <param name="versionFileDict">
        ///     Map of version number to migration file path.
        ///     The keys are required to be sorted to apply in order.
        /// </param>
        /// <remarks>
        ///     Throws ArgumentException if the version=>file dictionary does not give us the keys in order.
        /// </remarks>
